...
//           console.log(senderId); // Set default to the first sender ID
//         }
//         setLoading(false);
//       })
//       .catch(error => {
            ...
            {sentMessages.length > 0 ? (
              sentMessages.map((msg, index) => (
                <div key={index} className="border-b py-2">
                  ...
                  <p className="text-xs text-gray-500">Sent On: {new Date(msg.send_on).toLocaleString()}</p>
                  <p className={`text-xs ${msg.status === 'Sent' ? 'text-green-600' : 'text-red-600'}`}>
                    Status: {msg.status}
                  </p>
                </div>
              ))
