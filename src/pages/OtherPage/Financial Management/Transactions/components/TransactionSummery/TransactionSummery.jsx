  ...
  return (
    <div className="bg-white  p-4 shadow rounded-xl space-y-5 pb-10">
      ...
      <ul className="text-lg space-y-4">
        ...
        <li className="flex items-center justify-between gap-4">
          <b className="font-normal text-sm md:text-lg">From - To Date :</b><span className="text-sm md:text-base"> {startDate} -{" "}
          {endDate}</span>
        </li>
        <li className="flex items-center justify-between gap-4">
          <b className="font-normal text-sm md:text-lg">Income (Filtered) :</b>{" "}
          <span
            className={`${
              data?.income > 0 ? "text-green-500" : "text-red-500" 
            } text-sm md:text-base`}
          >
            {data?.income || 0} <span className="text-black">BDT </span>
          </span>
        </li>
        <li className="flex items-center justify-between gap-4">
          <b className="font-normal  text-sm md:text-lg">Expense (Filtered):</b>{" "}
          <span className="text-red-500 text-sm md:text-base">{data?.expense || 0} <span className="text-black">BDT </span></span>
        </li>
        <li className="flex items-center justify-between gap-4">
          <b className="font-normal  text-sm md:text-lg">Total Discount :</b>{" "}
          <span className="text-red-500 text-sm md:text-base">{data?.totalDiscount || 0} <span className="text-black">BDT </span></span>
        </li>
        <li className="flex items-center justify-between gap-4">
          <b className="font-normal  text-sm md:text-lg">Total Balance :</b>{" "}
          <span
            className={`${data?.total >0 ? "text-green-500" : "text-red-500"} text-sm md:text-base`}
          >
          ...
          method_summary?.map((item, index) => (
            <li className="flex items-center justify-between gap-3" key={index}>
              ...
              <span className="text-neutral-900 text-sm md:text-base">
                {parseInt(item?.totalIncome) - parseInt(item?.totalExpense)} <span className="text-black">BDT </span>
              </span>
            </li>
          ))}
      </ul>
