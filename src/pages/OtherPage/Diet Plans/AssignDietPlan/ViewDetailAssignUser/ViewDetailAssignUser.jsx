...
const ViewDetailAssignUser = ({ foodHabitId }) => {
    ...
    const fetchDetails = async () => {
      try {
        ...
        const questionResponse = await axiosSecure.get(
          `/diet-plan/food-habit-question/get?branch=${user?.branch}`
        );
        ...
        const response = await axiosSecure.post(
          `/diet-plan/get-food-habit/${foodHabitId}?branch=${user?.branch}`,
          schema
        );
        setIsLoading(false);
        setRequestUser(response?.data?.user);

        const foodHabitUserAnswer = response?.data?.foodHabitUserAnswer;
  ...
  return (
    <div className="md:w-full w-[90%] max-w-4xl mx-auto p-4 md:p-6 bg-white shadow-md rounded-lg my-10">
      ...
      <h2 className="text-xl md:text-2xl font-semibold mt-6">Food Habits</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        {foodHabit?.map((item, index) => (
          ...
          >
            <p className="flex flex-wrap gap-2 capitalize">
              <span className="font-bold">{item?.name}:</span> {item.value || "N/A"}
            </p>
          </div>
        ))}
      </div>
