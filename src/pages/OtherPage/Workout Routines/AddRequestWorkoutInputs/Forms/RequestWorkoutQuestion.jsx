...
function RequestWorkoutQuestion({ setIsShowModal, isShowModal }) {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosSecure.post(
        `/workout-routines/workout-habit-question/create`,
        {
          name,
          field_type,
          label,
      ...
      if (response.status === 200 || response.status === 201) {
        Swal.fire({
          title: "Success",
          text: "Workout question added successfully",
          icon: "success",
  ...
  return (
    <div className="bg-white p-5 w-[90%] md:p-8 rounded-xl md:w-[40%]">
      ...
      <button
        ...
        className="font-semibold bg-blue-500 text-white hover:bg-blue-600 px-4 py-2 rounded-lg ml-auto flex gap-3 items-center justify-center w-fit mt-12"
        disabled={loading}
      >
        {loading && (
...
export default RequestWorkoutQuestion;
