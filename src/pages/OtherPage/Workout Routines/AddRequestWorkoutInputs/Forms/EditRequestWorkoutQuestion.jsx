...
function EditRequestWorkoutQuestion({ setIsShowModal, isShowModal, targetId }) {
  const { user } = useAuth();
    ...
    const fetchPaymentMethodData = async () => {
      try {
        const response = await axiosSecure.get(
          `/workout-routines/workout-habit-question/get-single/${targetId}?branch=${user?.branch}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosSecure.put(
        `/workout-routines/workout-habit-question/update/${targetId}?branch=${user?.branch}`,
        {
          name,
          field_type,
          label,
      ...
      if (response.status === 200 || response.status === 201) {
        Swal.fire({
          title: "Success",
          text: "Workout Question updated successfully",
          icon: "success",
  ...
  return (
    <div className="bg-white p-5 w-[90%] md:p-8 rounded-xl md:w-[40%]">
      ...
      <button
        ...
        className="font-semibold bg-blue-500 text-white hover:bg-blue-600 px-4 flex items-center justify-center gap-3 py-2 rounded-lg ml-auto  w-fit mt-12"
        disabled={loading}
      >
        {loading && (
...
export default EditRequestWorkoutQuestion;
