...
const Addworkout = () => {
    ...
    const fetchWorkouts = async () => {
      try {
        const response = await axiosSecure.get(
          `/workouts/filter/${muscle}/${submuscle}/${equipment}?branch=${branch}`
        );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        setFilteredWorkouts(
          filteredWorkouts.filter((workout) => workout._id !== id)
        );
  ...
  const { paginatedData, paginationControls, rowsPerPageAndTotal } =
    Mpagination({ totalData: filteredWorkouts });
  ...
  return (
    <div className="px-2">
          ...
          <div className="flex mt-3 md:mt-0 justify-between">
            <div className="flex justify-end gap-4 items-center md:mb-4">
              ...
              <div className="flex md:gap-2 cursor-pointer items-center bg-yellow-500 text-white py-2 px-2 md:px-4 rounded-xl shadow hover:bg-yellow-600 transition duration-300">
                <button onClick={handleAddNew} className="font-semibold">
                  Add
                </button>
      ...
      <div className="text-sm px-2">{rowsPerPageAndTotal}</div>
      <div className="md:p-4 p-2 py-3">
        <div className="flex space-x-4 mb-4">
          <select
            ...
            className="border outline-none rounded-xl w-full px-1 py-1 md:py-2"
          >
            ...
            <tbody>
              {loading ? (
                ...
                paginatedData.map((workout) => (
                  <tr
                    key={workout._id}
                    className="hover:bg-slate-100 hover:rounded-xl transition duration-200 ease-in-out"
                  >
                    ...
                    <td className="px-4 py-3 block md:hidden">Pc view only</td>
                    ...
                    <td className="px-4 pt-10  flex items-center">
                      <button
