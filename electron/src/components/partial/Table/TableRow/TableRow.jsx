    ...
    if (daysDifference > 0) {
        ...
        <div>
          {daysDifference === 0 ? (
            ...
            <div className="min-w-max">
              ...
              <span className="text-sm">Expires in {daysDifference} {daysDifference === 1 ? 'day' : 'days'}</span>
            </div>
          )}
        </div>
      );
      ...
      return (
        <div className="min-w-max">
          ...
          <span className="text-sm"> Expired {daysAgo} {daysAgo === 1 ? 'day' : 'days'} ago</span>
        </div>
      );
    }
  };
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const res = await axiosSecure.delete(`/users/delete/${id}?branch=${branch}`);
      ...
      <tr className="border-b bg-white">
        <td className="w-full lg:w-auto">
          <div className="relative">
            ...
            <div
              onClick={() => {setIsShowNote(true); setTargetId(data._id); setNoteData(data?.notes || "")}}
        ...
        <td className="text-left space-y-1">
          <h5 className="font-bold text-base text-gray-600">{data?.member_id}</h5>
        ...
        <td className="text-left">
          <h3 className="text-xs md:text-base font-semibold">{data?.full_name}</h3>
        ...
        <td className="flex pt-8 flex-row-reverse justify-end text-left items-center">
          ...
          <button onClick={() => handleMemberDelete(data._id)} className="pr-1">
...
<div className="grid grid-cols-5 gap-6 p-6 shadow-xl rounded-lg bg-gradient-to-r from-blue-50 to-indigo-100 relative overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-2xl">
  ...
  <div className="col-span-3 flex flex-col justify-center space-y-2">
    <h2 className="text-2xl font-bold text-indigo-800">{data?.full_name || "N/A"}</h2>
    <p className="text-gray-600 font-medium">🆔 Member ID: {data?.member_id || "N/A"}</p>
    <p className="text-gray-600 font-medium">📞 Mobile: {data?.contact_no || "N/A"}</p>
    <p className="text-gray-600 font-medium">📧 Email: {data?.email || "N/A"}</p>
    ...
    <p className="text-gray-600 font-medium">
      ...
      {data?.expiredate && new Date(data.expiredate) > new Date()
        ? `(Expires in ${Math.floor(
            (new Date(data.expiredate) - new Date()) / (1000 * 60 * 60 * 24)
          )} days)`
        : data?.expiredate
        ? `(Expired ${Math.floor(
            (new Date() - new Date(data.expiredate)) / (1000 * 60 * 60 * 24)
          )} days ago)`
        : ""}
    </p>
  </div>
