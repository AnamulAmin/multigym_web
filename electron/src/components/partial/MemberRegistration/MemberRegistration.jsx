...
const schema = z.object({
  ...
  card_no: z.string().nonempty({ message: "Please enter card number" }),
});
...
function MemberRegistration({ setIsShow, isShow }) {
  ...
  const validateAdvancedFields = (data) => {
    ...
    if (!data.weight)
      errors.push({ field: "weight", message: "Please enter your weight!" });
    if (!data.blood_group)
      errors.push({
        field: "blood_group",
  ...
  return (
    <article
      className={`w-[90%] md:w-[65%] lg:w-[80%] rounded-xl bg-white my-7 transition-all duration-500 ${
